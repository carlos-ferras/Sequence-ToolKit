...
class MergeColors(Colors):
    show_error_message = QtCore.pyqtSignal(str)

    def __init__(self, parent=None):
        Colors.__init__(self, parent)

        self.color1_palette = self.getConfiguration('merge_color_1', 'GENSEC')
        self.color2_palette = self.getConfiguration('merge_color_2', 'GENSEC')
        self.color3_palette = self.getConfiguration('merge_color_3', 'GENSEC')
        self.color1.mouseDoubleClickEvent = partial(self.colorClicked, 'merge_color_1')
        self.color2.mouseDoubleClickEvent = partial(self.colorClicked, 'merge_color_2')
        self.color3.mouseDoubleClickEvent = partial(self.colorClicked, 'merge_color_3')

        self.fillValues()

    def colorClicked(self, color_name, event):
        color = QtGui.QColor(self.getConfiguration(color_name, 'GENSEC'))
            ...
            if not color.name() in [self.color1_palette, self.color2_palette, self.color3_palette]:
                ...
                self.show_error_message.emit(
                    QtCore.QCoreApplication.translate(
                        'color_dialog',
                        "Merge colors must be different."
                    )
                )

    def saveData(self):
        self.setConfiguration('merge_color_1', self.color1_palette, 'GENSEC')
        self.setConfiguration('merge_color_2', self.color2_palette, 'GENSEC')
        self.setConfiguration('merge_color_3', self.color3_palette, 'GENSEC')
