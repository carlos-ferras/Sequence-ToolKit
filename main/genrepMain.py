...
from dialogs.genrep.setup import classSetup
from dialogs.genrep.profile import classProfile 
...
class classGenRep(classTableBase):	
	def __init__(self,dir=False,parent=None):		
		...
		self.NMuestras=0
		classTableBase.__init__(self,'GenRep','pixmaps/genrep.png',dir)
	...
	def createXML(self):
		...
		self.myREP=None
		tabla=self.getallData()		
		self.myREP=createRLF.classREP(nmuestras=self.NMuestras,name=self.nombre,owner=self.propietario,n2flow=self.nitrogeno,doserate=self.dosis,extdoserate=self.dosisE,protocol=self.protocolo,status=self.STATUS,reader_id=self.id_lector,datecrea=self.datecrea)
	...
	def changeTheme(self,them):
		...
		self.active_bar.setStyleSheet('background:'+self.fondo[1])
	...
	def Apply_To(self):
		...
		self.apply_to_win=classApplyTo(send,self.form1)
				...
				if self.criterias_to[0][0]!='':	
					for column in range(self.comandos+1)[2:]:
						if item.text(column):
							...
							if data!='error':
								if criteria[1]=='Same':
									first.append (classIndex(i,column))
				...
				if len(self.criterias_to)>1!='':
					...
					if len(first)>1:
						for i in first:							
							...
							if data!='error':
								if criteria[1]=='Same':
									second.append (classIndex(i.row(),i.column()))
								elif criteria[1]=='Different':
									if str(data)!=str(criteria[2]):
										second.append (classIndex(i.row(),i.column()))
								elif criteria[1]=='Value':
					...
					if len(self.criterias_to)>2!='':		
						...
						if len(second)>1:
							for i in second:							
								...
								if data!='error':
									if criteria[1]=='Same':
										tercero.append (classIndex(i.row(),i.column()))
									elif criteria[1]=='Different':
										if str(data)!=str(criteria[2]):
											tercero.append (classIndex(i.row(),i.column()))
									elif criteria[1]=='Value':
						...
						if len(self.criterias_to)>3!='':		
							...
							if len(tercero)>1:
								for i in tercero:							
									...
									if data!='error':
										if criteria[1]=='Same':
											cuarto.append (classIndex(i.row(),i.column()))
										elif criteria[1]=='Different':
											if str(data)!=str(criteria[2]):
												cuarto.append (classIndex(i.row(),i.column()))
										elif criteria[1]=='Value':
	...
	def associationCriteria(self):
		...
		for i in self.parameters:
			send.append(self.enum_parameters	[i])
		self.association=classAssociation(send,self.consecutives,self.form1)
		self.association.pushButton_2.clicked.connect(self.association_ready)	
		
		
	def  data_setup(self):
		self.setup=classSetup(self.curve_to_show,self.show_tl,self.h_scale,self.h_min,self.h_max,self.h_great_unit,self.h_small_unit,self.unit,self.v_scale,self.v_min,self.v_max,self.v_great_unit,self.v_small_unit,self.signal,self.background,self.s_low,self.s_high,self.b_low,self.b_high,self.form1)
		self.setup.pushButton.clicked.connect(self.setup_ready)		
	

	def  data_profile(self):
		self.profile=classProfile(self.parameters,self.form1)
	...
	def create_graphic(self,X,Y):
		...
		self.canvas = classLienzo(X,Y,w,sl,sh,bl,bh,default,self.signal,self.background,self.h_min,self.h_max,self.h_great_unit,self.h_small_unit,self.v_min,self.v_max,self.v_great_unit,self.v_small_unit,self.fondo_graph,False,self.verticalLayoutWidget)
		try:
			if self.v_scale=='log':
		...
		self.xmin1_sb = classValuesSpinBox(self.spin_values)		
		...
		self.xmax1_sb =classValuesSpinBox(self.spin_values)	
		...
		self.xmin2_sb = classValuesSpinBox(self.spin_values)	
		...
		self.xmax2_sb =classValuesSpinBox(self.spin_values)	
	...
	def fillActions(self):
		...
		self.m_anim = classAnimation(self.down_area, 'pos')
				...
				if self.criterias[0][0]!='':	
					for column in range(self.comandos+1)[2:]:
						if item.text(column):
							...
							if data!='error':
								if criteria[1]=='Same':
									first.setdefault(data, []).append (classIndex(i,column))
								elif criteria[1]=='Different':
									if str(data)!=str(criteria[2]):
										first.setdefault(data, []).append (classIndex(i,column))
								elif criteria[1]=='Value':
									...
									if '*' in value:
										...
										while ('*' in value):
											...
											data=data[p+1:]		
										if data==value:
											first.setdefault(str(criteria[2]), []).append (classIndex(i,column))											
									else:
										if type(data)==float:
											try:
												value=float(value)
					...
					for list in first.values():
						...
						if len(list)>1:
							for i in list:							
								...
								if data!='error':
									if criteria[1]=='Same':
										second.setdefault(data, []).append (classIndex(i.row(),i.column()))
									elif criteria[1]=='Different':
										if str(data)!=str(criteria[2]):
											second.setdefault(data, []).append (classIndex(i.row(),i.column()))
									elif criteria[1]=='Value':
										...
										if '*' in value:
											...
											while ('*' in value):
												...
												data=data[p+1:]		
											if data==value:
												second.setdefault(str(criteria[2]), []).append (classIndex(i.row(),i.column()))										
										else:
											if type(data)==float:
												try:
													value=float(value)
							...
							for list in second.values():
								...
								if len(list)>1:
									for i in list:							
										...
										if data!='error':
											if criteria[1]=='Same':
												tercero.setdefault(data, []).append (classIndex(i.row(),i.column()))
											elif criteria[1]=='Different':
												if str(data)!=str(criteria[2]):
													tercero.setdefault(data, []).append (classIndex(i.row(),i.column()))
											elif criteria[1]=='Value':
												...
												if '*' in value:
													...
													while ('*' in value):
														...
														data=data[p+1:]		
													if data==value:
														tercero.setdefault(str(criteria[2]), []).append (classIndex(i.row(),i.column()))										
												else:
													if type(data)==float:
														try:
															value=float(value)
									...
									for list in tercero.values():
										...
										if len(list)>1:
											for i in list:							
												...
												if data!='error':
													if criteria[1]=='Same':
														cuarto.setdefault(data, []).append (classIndex(i.row(),i.column()))
													elif criteria[1]=='Different':
														if str(data)!=str(criteria[2]):
															cuarto.setdefault(data, []).append (classIndex(i.row(),i.column()))
													elif criteria[1]=='Value':
														...
														if '*' in value:
															...
															while ('*' in value):
																...
																data=data[p+1:]		
															if data==value:
																cuarto.setdefault(str(criteria[2]), []).append (classIndex(i.row(),i.column()))										
														else:
															if type(data)==float:
																try:
																	value=float(value)
...
class classBackgroundColorDelegate(QtGui.QStyledItemDelegate):
	def __init__(self, parent,columns,colorear,dejar,color,fondo):
		super(classBackgroundColorDelegate, self).__init__(parent)
	...
	def paint(self, painter, option, index):
		if index.column() in self.columns and self.colorear:
			...
			super(classBackgroundColorDelegate, self).paint(painter, option, index)
...
class classAnimation(QtCore.QPropertyAnimation):
	LinearPath, CirclePath = range(2)

	def __init__(self, target, prop):
		super(classAnimation, self).__init__(target, prop)

		self.setPathType(classAnimation.LinearPath)
	...
	def updateCurrentTime(self, currentTime):
		if self.m_pathType == classAnimation.CirclePath:
			...
			super(classAnimation, self).updateCurrentTime(currentTime)
			
			
class classValuesSpinBox(QtGui.QSpinBox):
    def __init__(self, values, parent=None):
        super(classValuesSpinBox, self).__init__(parent)
        if values==[]:
			values=['0,0000']
