...
class classGenSec(classTableBase): 
		def __init__(self,dir=False, parent=None):			
			classTableBase.__init__(self,'GenSec','pixmaps/gensec.png',dir)
		...
		def createXML(self):
			...
			self.mySEQ=createSLF.classSEQ(nmuestras=self.nmuestras,name=self.nombre,owner=self.propietario,n2flow=self.nitrogeno,doserate=self.dosis,extdoserate=self.dosisE,protocol=self.protocolo,reader_id=self.id_lector,datecrea=self.datecrea)
		...
		def previusly(self,temp=False):
			...
			try:
				...
				pass
			self.priview=classPriview(self.form1)			
			self.createXML()				
		...
		def itemAction(self,item,column):
			...
			if column==1:
				...
				if command=='Pre-Heat':
					...
					self.oper=classOperations(self.form1)
