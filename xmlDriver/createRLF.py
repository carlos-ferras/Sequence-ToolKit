...
class classREP:
	...
	def createSample(self,sample_id):
		...
		sample=classSample(sample_id).Sample_ID
		if len(s_ids)>0:
			for sam in s_ids:
	...
	def createProcessOrder(self,Sample_ID,process_order_id,ptype,dtype,curves,parameters):
		...
		processO=classProcessOrder(process_order_id,ptype,dtype,curves,parameters)
	...
	def createCurve(self,num,signal,background,Conteos_SG, Canal_inf_SG, Canal_sup_SG, Conteos_BG, Canal_inf_BG,Canal_sup_BG):
		curve=classCurve(num,signal,background,Conteos_SG, Canal_inf_SG, Canal_sup_SG, Conteos_BG, Canal_inf_BG,Canal_sup_BG)
		self.setDateMod()
		return curve.Curve	
